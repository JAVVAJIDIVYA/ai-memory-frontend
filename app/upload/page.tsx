...
export default function UploadPage() {
  ...
  const uploadMutation = useMutation({
    mutationFn: () => {
      ...
      if (mode === 'voice') return notesService.uploadVoice(file!, subject || undefined, topic || undefined, voiceTitle || undefined);
      return notesService.uploadFile(file!, subject || undefined, topic || undefined);
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['notes'] });
      // Voice takes longer to process — give user time to read the message
      setTimeout(() => router.push('/'), mode === 'voice' ? 4000 : 2000);
    },
  });
      ...
      return (
        <div className="space-y-2">
          ...
          <p className="text-xs" style={{ color: 'var(--text-muted)' }}>
            YouTube transcripts or web page text are extracted automatically.
          </p>
        </div>
      );
    }
        ...
        {mode === 'voice' && (
          <div>
            <label className="block text-sm font-medium mb-2" style={{ color: 'var(--text-secondary)' }}>
              Note Title <span style={{ color: 'var(--text-muted)', fontWeight: 400 }}>(optional)</span>
            </label>
            <input
              type="text"
              placeholder="e.g. Lecture 3 — Operating Systems"
              value={voiceTitle}
          ...
          <div className="flex flex-col items-center gap-4">
            ...
            {file ? (
              ...
              <div>
                ...
                <p className="text-sm mt-1" style={{ color: 'var(--text-muted)' }}>
                  {mode === 'voice' ? 'MP3, WAV, M4A, FLAC, OGG, WebM' : 'PDF, DOCX, TXT, PNG, JPG, WebP'}
                </p>
              </div>
            )}
          </div>
    ...
    <div className="max-w-2xl mx-auto space-y-8">
      ...
      >
        ...
        <p className="text-indigo-100 mt-2">
          PDF, DOCX, TXT, images (OCR), audio/voice notes, or YouTube links.
        </p>
      ...
      >
        {([
          ...
          { key: 'voice' as const, icon: Mic, label: 'Audio / Voice' },
        ]).map(({ key, icon: Icon, label }) => (
          <button
            key={key}
            type="button"
            onClick={() => { setMode(key); setFile(null); setVoiceTitle(''); }}
      ...
      >
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          >
            {uploadMutation.isPending ? (
              <><Loader2 className="w-5 h-5 animate-spin" />{mode === 'voice' ? 'Uploading Audio...' : 'Uploading & Processing...'}</>
            ) : (
              mode === 'voice' ? 'Upload & Transcribe' : 'Upload & Process'
            )}
          </button>
        </form>

        {uploadMutation.isSuccess && (
          <div className="mt-6 p-4 rounded-xl flex items-start gap-3" style={{ background: '#ecfdf5', border: '1px solid #6ee7b7', color: '#059669' }}>
            <CheckCircle2 className="w-5 h-5 mt-0.5 flex-shrink-0" />
            <div>
              <p className="font-semibold">
                {mode === 'voice' ? 'Audio uploaded successfully!' : 'Uploaded! AI processing started.'}
              </p>
              {mode === 'voice' ? (
                <p className="text-sm mt-1 opacity-80">
                  Transcription is running in the background — this can take up to a minute. Check your notes shortly.
                </p>
              ) : (
                <p className="text-sm mt-1 opacity-80">Redirecting to dashboard...</p>
              )}
            </div>
          </div>
        )}
        ...
        {uploadMutation.isError && (
          <div className="mt-6 p-4 rounded-xl flex items-center gap-3" style={{ background: '#fef2f2', border: '1px solid #fca5a5', color: '#dc2626' }}>
            ...
            <p className="font-medium">{(uploadMutation.error as ApiError)?.response?.data?.detail || 'Upload failed. Please try again.'}</p>
          </div>
        )}
      </div>
    </div>
